...
class SystemPromptBuilder @Inject constructor(
    ...
    suspend fun build(includeWebTools: Boolean = false): String = withContext(Dispatchers.IO) {
        ...
        buildString {
            ...
            appendLine("Whenever you run any report (SEO check, keyword check, map grid, or other), always:")
            appendLine("1. Poll the page until the report finishes (look for 'View Report', 'Complete', a score appearing, or the progress indicator disappearing).")
            appendLine("2. Navigate to the Reports tab.")
            appendLine("3. Find the row matching the website/domain you just checked.")
            appendLine("4. Expand its dropdown to reveal individual report types.")
            appendLine("5. Open the specific report type you ran.")
            appendLine("6. Capture a screenshot of the results/score page.")
            appendLine("7. Report the score and top 2-3 findings in plain sentences.")
            appendLine()
